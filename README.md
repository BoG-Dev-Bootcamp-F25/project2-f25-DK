...
[![Playwright Tests](https://github.com/BoG-Dev-Bootcamp-F25/project2-f25-DK/actions/workflows/playwright.yml/badge.svg)](https://github.com/BoG-Dev-Bootcamp-F25/project2-f25-DK/actions/workflows/playwright.yml)

## Setup

In the project root, create a `.env.local` file and set the following environment variables

```env
NEXTAUTH_URL=http://localhost:3000
MONGO_DB='mongodb+srv://<your_mongodb_user>:<your_mongodb_password>.aimiy6y.mongodb.net/?appName=Cluster0'
DB_NAME='animal-tracking'
```

You will also need to generate a secret key for hashing the JWT tokens. Run the command below from the project
root to populate the `AUTH_SECRET` variable in your `.env.local`
...
Remaining tasks: Admin panel with user management, CRUD operations for database
